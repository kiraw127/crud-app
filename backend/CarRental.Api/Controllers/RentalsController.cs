...
public class RentalsController(AppDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<Rental>>> GetRentalsAsync() => Ok(await db.Rentals.Include(rental => rental.Car).OrderByDescending(rental => rental.Id).ToListAsync());

    [Authorize, HttpGet("me")]
    public async Task<ActionResult<IEnumerable<Rental>>> GetMyRentalsAsync() { var email=User.FindFirstValue(ClaimTypes.Email); var user=await db.Users.SingleOrDefaultAsync(item=>item.Email==email); if(user is null)return Unauthorized(); return Ok(await db.Rentals.Include(rental=>rental.Car).Where(rental=>rental.UserId==user.Id).OrderByDescending(rental=>rental.Id).ToListAsync()); }
